...
#SBATCH --job-name=attention_unet_CXR  # Job name
#SBATCH --output=attention_unet_CXR_%j.out  # Output file name (%j = job ID)
...
singularity exec --nv \
    ...
    /bin/bash -c "source /ext3/env.sh; python training.py"